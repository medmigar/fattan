...
function affDmt() { 
    ...
    if(fileInput.files[0]==undefined){
        mybody.innerHTML ="<p style='width: 90%;text-align:center;font-size:18px;'>On doit choisir un fichier</p>";modal.style.display =
        "block";
    }else{
        var nom=fileInput.files[0].name;
        var ary = nom.split(".");
        if(ary[1]=="php"||ary[1]=="html"||ary[1]=="txt"){
         document.getElementById('file').files[0].text().then(t => document.getElementById('lab').textContent=t);
